...
describe('loadTasks', () => {
  ...
  afterEach(() => {
    [JS_FILE, CJS_FILE, TS_FILE].forEach(f => { if (fs.existsSync(f)) fs.unlinkSync(f); });
  });
  afterAll(() => {
  ...
  it('loads tasks from skier.tasks.js', async () => {
    fs.writeFileSync(JS_FILE, `exports.tasks = [{ name: 'test', config: {}, run: async () => ({}) }];`);
  ...
  it('loads tasks from skier.tasks.cjs if .js is missing', async () => {
    fs.writeFileSync(CJS_FILE, `exports.tasks = [{ name: 'cjs', config: {}, run: async () => ({}) }];`);
    const tasks = await loadTasks(TEST_DIR);
    expect(tasks[0].name).toBe('cjs');
  });

  it('loads tasks from skier.tasks.ts if .js and .cjs are missing', async () => {
    fs.writeFileSync(TS_FILE, `exports.tasks = [{ name: 'ts', config: {}, run: async () => ({}) }];`);
    const tasks = await loadTasks(TEST_DIR);
    expect(tasks[0].name).toBe('ts');
  });

  it('throws if no tasks file is found', async () => {
    await expect(loadTasks(TEST_DIR)).rejects.toThrow('Could not find a skier.tasks.js, skier.tasks.cjs, or skier.tasks.ts file');
  });
