...
function createRenderer(headings: Heading[]): InstanceType<typeof marked.Renderer> {
  ...
  // Fenced code blocks render through the structured `<figure class="code-block">`
  // markup (language + optional filename data attributes) shared with snippet
  // transclusion — see `renderCodeBlock`.
...
// Register the callout / admonition block extension once, globally. It carries
// its own tokenizer + renderer, so it composes with the per-render heading/code
// renderer built by `createRenderer`.
 ...
 *
 * `@include` snippet directives are resolved against real files before parsing;
 * pass `includeBaseDir` to control where those relative paths resolve.
 */
